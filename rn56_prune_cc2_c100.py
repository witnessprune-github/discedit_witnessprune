...
def create_class_conditional_datasets(root, train=True, transform=None):
    # Load the CIFAR100 dataset with transformations
...
class WitnessPrune:
    def __init__(self, model, layer, layer2, budget):
        if not 0 <= budget <= 1:
    ...
    def Prune(self, pruning_mask, basic_mask, model):
        conv_layer = self.layer
        bn_layer = self.layer2
        # print(conv_layer.weight.shape, conv_layer.bias.shape, bn_layer.weight.shape, bn_layer.bias.shape)
        # Prune the bias of the convolutional layer using basic_mask
...
def main():
    with torch.no_grad():
        f = open('pruning_accs_rn56c100.txt', 'a')
        ...
        class_filters_pruned = np.zeros([100,1])
        for class_id in range(100):
            ...
            model.to(device)
            model.eval()
            layers_bn=  get_layers(model.layer3)
            ...
            for ll in range(len(layers_bn)):
                print(ll,class_id)
                _, layer = layers_bn[ll]
                ...
                _, layer2 = layers_conv[ll]
                print(layer2.weight.shape)
                # print(layer2.weight.shape)
                pruner = WitnessPrune(model, layer2, layer, p_ratio)
                print(pruner.layer.weight.shape, pruner.layer2.weight.shape)
                basic_mask = pruner.get_basic_masks(rr)
                pruning_mask = pruner.build_pruning_mask(basic_mask)
                pruner.Prune(pruning_mask, basic_mask, model)
        ...
        # print(f"Accuracy on non-class dataset: {non_class_accuracy * 100:.2f}%")
            ...
            f.write(f"c_id: {class_id}, cacc_old = {acc_class}, ncacc_old = {acc_nonclass}, cacc_new = {class_accuracy}, ncacc_new = {non_class_accuracy}, pnum = {class_filters_pruned[class_id]}\n")
