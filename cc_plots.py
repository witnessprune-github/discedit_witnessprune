...
def main():
    with torch.no_grad():
        ...
        # conv_dict_layers = [0,3,7,10,14,17,20,24,27,30,34,37,40]
        ...
        tv_scores = {}
        ylims = [.2,.15,.15,.15,.15,.15,.15,.15,.15,.25,.25,.25,.95]
        ...
        for ll in range(len(conv_dict)):
            ...
            for class_id in range(10):
                ...
                class_scores[class_id] = rr
                filename = './vgg16_c10/vgg16c10_later'+str(ll)+'_class'+str(class_id)
                # plt.ylim(0,ylims[ll])
                
                plt.plot(rr, linewidth=3.0)
                plt.xticks(fontsize=18, rotation=90)
                plt.savefig(filename)
                plt.close()
        
            tv_scores[ll] = class_scores

    with open('vgg16_c10_scores.pkl', 'wb') as f:
