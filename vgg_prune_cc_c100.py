...
def create_class_conditional_datasets(root, train=True, transform=None):
    ...
    for class_id in range(100):  # CIFAR10 has 10 classes
...
def main():
    with torch.no_grad():
        f = open('pruning_accs_vgg16c100.txt', 'a')
        ...
        test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=2000, num_workers=num_workers)
        ...
        for class_id in range(100):
            ...
            with torch.no_grad():
                image,label = next(dataiter_test)
                output = model(image)

                print(image.shape, output.shape)

                acc = accuracy(output, label, topk=(1,))

                print(acc)

            
            class_loader = DataLoader(class_datasets[class_id], batch_size=256, shuffle=True)
            non_class_loader = DataLoader(non_class_datasets[class_id], batch_size=4096, shuffle=True)
            test_class_loader = DataLoader(test_class_datasets[class_id], batch_size=64, shuffle=True)
            test_non_class_loader = DataLoader(test_non_class_datasets[class_id], batch_size=7500, shuffle=True)
            device='cuda:0'
            model.to(device)
            model.eval()
            ...
            for ll in range(len(conv_dict[6:13])):
                print(ll,class_id)
