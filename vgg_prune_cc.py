...
def test_accuracy(model, test_loader, device='cuda'):
    """Evaluate model accuracy over the entire test dataset.
    ...
    Returns:
        List[float]: Accuracy for each topk prediction.
    """
...
def main():
    with torch.no_grad():
        ...
        for class_id in range(10):
            ...
            test_class_loader = DataLoader(test_class_datasets[class_id], batch_size=32, shuffle=True)
            test_non_class_loader = DataLoader(test_non_class_datasets[class_id], batch_size=256, shuffle=True)
            ...
            for ll in range(len(conv_dict)):
                print(ll,class_id)
                layer = model.features[conv_dict[ll]]
                # print(ll)
                ...
                layer2 = model.features[conv_dict2[ll]]
                ...
                pruner.Prune(pruning_mask, basic_mask, model, conv_dict2[ll])
