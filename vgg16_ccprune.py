...
def test_accuracy(model, test_loader, device='cuda'):
    ...
    topk_accuracies = []  # List to store the top-k accuracies for all batches
    
    # We assume topk values are consistent across all calls, define as needed
    topk = (1,)  # Modify this if you want to compute top-1, top-5 etc.
    
    with torch.no_grad():
        for inputs, labels in test_loader:
            inputs, labels = inputs.to(device), labels.to(device)
            outputs = model(inputs)
            accuracies = accuracy(outputs, labels, topk=topk)
            topk_accuracies.append(accuracies)
    
    # Average the accuracies across all batches
    avg_accuracies = [torch.mean(torch.cat([topk_accuracies[b][i] for b in range(len(topk_accuracies))]))
...
def main():
    with torch.no_grad():
        ...
        class_filters_pruned = np.zeros([10,1])
        for class_id in range(10):
            ...
            test_class_loader = DataLoader(test_class_datasets[class_id], batch_size=640, shuffle=True)
            test_non_class_loader = DataLoader(test_non_class_datasets[class_id], batch_size=8192, shuffle=True)
            device='cuda:0'
            model.to(device)
            model.eval()
            tacc = test_accuracy(model, test_loader, device='cuda')
            print(tacc)
            ...
            for ll in range(len(conv_dict[5:13])):
                ...
                p_ratio = 20/len(rr)
                print(p_ratio, p_ratio*len(rr))
                class_filters_pruned[class_id] += p_ratio*len(rr)
            ...
            class_accuracy = test_accuracy(model, test_class_loader, device)
            print(class_accuracy.dtype)
        # print(f"Accuracy on class-specific dataset: {class_accuracy * 100:.2f}%")
            class_diff = class_accuracy - acc_class

            non_class_accuracy = test_accuracy(model, test_non_class_loader, device)
            non_class_diff = non_class_accuracy - acc_nonclass
            print(class_accuracy, non_class_accuracy, class_filters_pruned[class_id])
        # print(f"Accuracy on non-class dataset: {non_class_accuracy * 100:.2f}%")
            ...
            f.write(f"c_id: {class_id}, cacc_old = {acc_class}, ncacc_old = {acc_nonclass}, cacc_new = {class_accuracy}, ncacc_new = {non_class_accuracy}, cacc_diff = {class_diff}, ncacc_diff = {non_class_diff}, pnum = {class_filters_pruned[class_id]} \n")
            # f.write(f"cid: {class_id}, class_filters_pruned = {class_filters_pruned[class_id]}\n")
            # for i in conv_dict2:
            #     print(model.features[i+1].running_mean,model.features[i+1].running_var)
            # wait = input("Press Enter to continue.")
